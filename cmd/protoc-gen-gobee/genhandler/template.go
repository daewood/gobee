...
func (d *{{$svc.GetName}}Desc) RegisterHTTP(mux transport.Router) {
	...
	{{range $b := $m.Bindings}}
	mux.HandleFunc("/"+pattern_gobee_{{$svc.GetName}}_{{$m.GetName}}_{{$b.Index}}, func(w http.ResponseWriter, r *http.Request) {
...
{{range $m := $svc.Methods}}
{{range $b := $m.Bindings}}
	pattern_gobee_{{$svc.GetName}}_{{$m.GetName}}_{{$b.Index}} = strings.Join({{$b.PathTmpl.Pool | printf "%#v"}},"/")
{{end}}
{{end}}
{{end}}
)
